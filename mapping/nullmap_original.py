...
plot_layout = {
    'm': (-35, "red", 16),   # mapped
    's': (-15, "c", 10),     # start search
    'e': (-25, "blue", 10),  # end of letter
}
...
def main():
    ...
    freq_vect[freq_vect > 300] = 300  # limit max value to 300 Hz (numpy)
    ...
    xinch = xdim * 0.005   # zoom factor for x-axis (in inches)
    ...
    print("db_text:", text)
    print("timing relation", int(letter_length))
    ...
    textpos = np.linspace(340, int(xdim-370), len(text))
...
def map(preds, text, letter_length):
    # letter_length is approx. ms per letter
    trigger = 0.001  # minimum probability
    startpos = 80 # slices, time in audio

    letter_slices = int(letter_length / 5 )
    ...
    curpos = startpos   # Position is in slices of 5ms !!!
    prev_ltr = ''
    for ndx, ltr in enumerate(text):
       ...
       # if currpos is too far behind a calculated letter position,
        ...
        calcpos = (ndx * letter_length + 400) / 5 # /5 for slices
        ...
        identical = count_identical_letters(text, ndx)

        print(f"from {curpos*5:5.1f} ms scan for ltr '{ltr}' ", end=" ")
        seq = avg_tab[ltr]

        # search for the prediction
        for p in range(curpos, curpos + letter_slices * identical):
            ...
            raise Exception(f"not found {ltr} after {curpos*5}")


        # search end of current letter
        curpos += letter_slices
...
def __map(avg_tab, text, time_factor):
    ...
    print(text)
    curpos = 80   # Position is in slices of 5ms !!!
    prev_ltr = ''
    repeat = 0
    ...
    for ndx, ltr in enumerate(text):
        ...
        # if currpos is too far behind a calculated letter position,
        # then advance the currpos
        # calcpos = (ndx * time_factor) / 5 # /5 for slices
        # if calcpos - curpos > limit_search:
        #     print(f"curpos {curpos*5} too far behind {int(calcpos*5)}", end = ' ')
        #     curpos = int(calcpos - limit_search/2)
        #     print(f" move to {curpos*5}  .. (ms)")
        ...
        G.mapping_tab.append((f"<{ltr}", curpos*5, 's'))
        seq = avg_tab[ltr]
        ...
        at_least = curpos + repeat * minlen
        # at_least: the letter can not end before this
        while seq[curpos] >= trigger or curpos < at_least:
            curpos += 1

        # probability fell below the trigger threshold
        curpos -= 5
        ltrpos = (curpos * 5 + ltr_start)/2
        print(f"end at {curpos * 5: 5d} final pos: {ltrpos} repeat: {repeat}")
        G.mapping_tab.append((f"{ltr}>", curpos * 5, 'e'))
        G.mapping_tab.append((ltr, ltrpos, 'm'))
        G.mapped_letters.append((ltr, ltr_start, curpos*5))
